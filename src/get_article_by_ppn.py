...
iter_EVENT_BASED_URL = f"https://jsru.kb.nl/sru/sru?version=1.2&operation=searchRetrieve&x-collection=DDD_artikel&recordSchema=indexing&startRecord=%s&maximumRecords=100&query=\"%s\" AND (date within \"%s %s\")"
...
def get_article_by_event(event_set, OUTPUT_DIR):
    for event in event_set:
        ...
        for start in range(1, total_nr_results+1, 100):
